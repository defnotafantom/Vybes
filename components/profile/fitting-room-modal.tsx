...
import { ShoppingBag, Coins, Check, Lock, Image as ImageIcon, X } from 'lucide-react'
...
export function FittingRoomModal({ isOpen, onClose }: FittingRoomModalProps) {
  ...
  const handlePurchase = async (itemId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: t('avatar.purchaseSuccess'),
          description: t('avatar.purchaseSuccessDesc'),
        })
        setCoins(data.coins)
        ...
        toast({
          title: t('toast.error'),
          description: data.error || t('avatar.purchaseError'),
          variant: 'destructive',
        })
      }
    } catch (error) {
      ...
      toast({
        title: t('toast.error'),
        description: t('avatar.purchaseError'),
        variant: 'destructive',
      })
    } finally {
      setPurchasing(null)
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="relative bg-gradient-to-br from-white/95 via-sky-50/50 to-blue-50/50 dark:from-gray-800/95 dark:via-sky-900/30 dark:to-blue-900/30 backdrop-blur-xl rounded-3xl shadow-2xl border-2 border-sky-200/50 dark:border-sky-800/50 max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              {/* Close Button */}
              <button
                onClick={onClose}
              ...
              <div className="p-6 border-b border-sky-200 dark:border-sky-800">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-3xl font-black bg-gradient-to-r from-sky-600 to-blue-600 bg-clip-text text-transparent">
                      Fitting Room
                    </h2>
